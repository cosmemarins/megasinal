...
const UNDERSCORE_INDEXES = [indexKeys.MINI_TICKER, indexKeys.LAST_CANDLE, indexKeys.PREVIOUS_CANDLE, indexKeys.LAST_ORDER, indexKeys.WALLET];

function getShortestInterval(indexes) {
    if (!indexes || !indexes.length) return "1m";

    const allIntervals = indexes.map(ix => {
        if (UNDERSCORE_INDEXES.some(uix => new RegExp(`^([A-Z]+:${uix})`).test(ix))) return "1m";
...
async function backtestCandle(i, ohlc, userId, indexes, symbol, interval) {
    ...
    let miniTickerResult = await hydra.updateMemory(symbol, indexKeys.MINI_TICKER, null, lastCandle, true, userId);
    miniTickerResult = miniTickerResult ? miniTickerResult.filter(r => r) : false;
    if (miniTickerResult && miniTickerResult.length) {
        logger(`backtest-${userId}`, miniTickerResult);
        results.push(...miniTickerResult);
    }
...
async function doBacktest(req, res, next) {
    ...
    backtest.endBase = await hydra.getMemory(`${symbol.base}:WALLET_${userId}`, null, null, userId);
    backtest.endQuote = await hydra.getMemory(`${symbol.quote}:WALLET_${userId}`, null, null, userId);
