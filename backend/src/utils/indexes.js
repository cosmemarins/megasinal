...
function insideCandle(ohlc, bars = 1) {
    if(ohlc.high.length <= bars) return false;
...
function ADX(ohlc, period = 14) {
    if(ohlc.high.length <= period) return false;
...
function AO(ohlc, fast = 5, slow = 34) {
    if(ohlc.high.length <= fast || ohlc.high.length <= slow) return false;
...
function KST(closes, rocPer1 = 10, rocPer2 = 15, rocPer3 = 20, rocPer4 = 30, smarocPer1 = 10, smarocPer2 = 10, smarocPer3 = 10, smarocPer4 = 15, signal = 3) {
    if(closes.length <= rocPer1 || closes.length <= rocPer2 || closes.length <= rocPer3 || closes.length <= rocPer4
       || closes.length <= smarocPer1 || closes.length <= smarocPer2 || closes.length <= smarocPer3 || closes.length <= smarocPer4) return false;
...
function MACD(closes, fastPeriod = 12, slowPeriod = 26, signalPeriod = 9) {
    if(closes.length <= fastPeriod
        ...
        || closes.length <= signalPeriod) return false;
...
function StochRSI(closes, dPeriod = 3, kPeriod = 3, rsiPeriod = 14, stochasticPeriod = 14) {
    if(closes.length <= dPeriod
        || closes.length <= kPeriod
        || closes.length <= rsiPeriod
        || closes.length <= stochasticPeriod) return false;
