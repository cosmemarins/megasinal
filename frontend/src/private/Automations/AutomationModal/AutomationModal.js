...
function AutomationModal(props) {
    ...
    useEffect(() => {
        ...
        if (automation.symbol.startsWith('*'))
            ...
            getSymbol(automation.symbol).then(symbolObj => setSymbol(symbolObj));
    }, [automation.symbol])
        ...
        getIndexes()
            .then(indexes => {
                ...
                if (isWildcard) {
                    ...
                    const baseWallet = indexes.find(ix => ix.variable.startsWith('WALLET') && symbol.base === ix.symbol);
                    if (baseWallet) filteredIndexes.splice(0, 0, baseWallet);

                    const quoteWallet = indexes.find(ix => ix.variable.startsWith('WALLET') && symbol.quote === ix.symbol.replace('*', ''));
                            ...
                            <div className="row">
                                <div className="col-md-7">
                                    <div className="form-group">
                                        ...
                                        <SelectSymbol onChange={onInputChange} symbol={automation.symbol} onlyFavorites={false} disabled={automation.id > 0} showAny={true} />
                                    </div>
                                </div>
                            </div>
                            <div className="row">
