...
function QuantityInput(props) {
    ...
    function onCalcClick(event) {
        let qty;
        if (props.side === 'SELL') {
            qty = props.wallet.base.qty;
        } else {
            const quoteAmount = parseFloat(props.wallet.quote.qty);
    ...
    const quantityInput = useMemo(() => (
        <div className="form-group">
            ...
            <div className="input-group">
                ...
                <input id={props.id} ref={inputQuantity} type="number" value={props.quantity} className="form-control" placeholder={props.symbol.minLotSize} onChange={props.onChange} required />
            </div>
        </div>
    ), [props.wallet, props.quantity, props.symbol, props.price]);
